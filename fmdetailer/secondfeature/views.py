...
CSV_HEADER = [
    ...
    'passing', 'penalty_taking', 'tackling', 'technique', 'aggression', 
    'anticipation', 'bravery', 'composure', 'concentration', 'decisions', 
    'determination', 'flair', 'leadership', 'off_the_ball', 'positioning', 
    'teamwork', 'vision', 'work_rate', 'acceleration', 'pace', 'agility', 
    'balance', 'jump', 'stamina', 'strength', 'natural_fitness', 'gk_diving', 
    'gk_handling', 'gk_kicking', 'gk_reflexes', 'gk_positioning', 'aerial_reach', 
    'command_of_area', 'communication', 'eccentricity', 'handling', 'kicking', 
    'one_on_ones', 'punching', 'reflexes', 'throwing'
]
# Kolumny, które są atrybutami numerycznymi
NUMERIC_COLS = [col for col in CSV_HEADER if col not in ['name', 'position', 'best_role']]

# ----------------------------------------------------------------------
# 2. FUNKCJE POMOCNICZE (KALKULACJE)
# ----------------------------------------------------------------------

def fit_player(player_series, role, roles_weight):
    """(POPRAWIONA) Oblicza dopasowanie gracza (Pandas Series) do danej roli."""
    ...
    for attr, weight in weights.get('key', {}).items():
        # POPRAWKA: Użyj .lower(), aby dopasować do nagłówka CSV (np. 'Dribbling' -> 'dribbling')
...
def get_position_group(position_str):
    """Konwertuje string pozycji gracza na ogólną grupę (np. 'DEF')."""
...
def process_uploaded_csv_to_rows(uploaded_file):
    """
    Przetwarza JEDEN wgrany plik CSV na listę wierszy (list), 
    automatycznie obliczając 'best_role'.
    """
    try:
        # Odczytaj plik do DataFrame
        try:
        ...
        if df.empty:
            return [], None # Pusty plik to nie błąd
        ...
        # Obliczanie 'best_role' dla każdego gracza
        for index, player_series in df.iterrows():
            ...
            # Tworzenie finalnego wiersza jako lista
...
# ----------------------------------------------------------------------
# 3. NOWA LOGIKA PORÓWNAWCZA (Twoje funkcje + poprawki)
# ----------------------------------------------------------------------

def fit_to_role(role, df, roles_weight):
    """
    (NOWA FUNKCJA) Funkcja pomocnicza wymagana przez 'find_similar'.
    Oblicza dopasowanie dla każdego gracza w DataFrame do JEDNEJ, konkretnej roli
    i zapisuje wynik w nowej kolumnie 'fit_ROLA'.
    """
    col_name = f'fit_{role}' # Używamy małych liter
...
def find_similar(player_name, df, roles_weight):
    """
    (TWOJA FUNKCJA - POPRAWIONA) Zwraca zawodników podobnych do wybranego.
    Tolerancja jest teraz na stałe ustawiona na 10.
    """
    
    # --- ZMIANA: Tolerancja ustawiona na stałe ---
    tolerance = 10.0

    # Używamy 'name' i 'best_role'
    ...
    # Usuń gracza, którego porównujemy, z listy wyników
...
def main(request):
    """Renderuje stronę główną z nowym formularzem."""
    return render(request, "main.html")

def run_similarity_analysis(request):
    """
    Główny widok. Obsługuje 2 pliki, łączy je, uruchamia 'find_similar'
    i renderuje stronę wyników 'results.html' Z NOWĄ LOGIKĄ KONTEKSTU.
    """
    ...
    # Przetwórz plik 2 (Baza do porównania)
    ...
    # Stwórz jeden, połączony DataFrame
    all_rows = target_rows + base_rows
    df = pd.DataFrame(all_rows, columns=CSV_HEADER)

    # Konwertuj kolumny atrybutów na numeryczne (kluczowe!)
    ...
    # --- NOWA LOGIKA PRZYGOTOWANIA DANYCH DO TABELI ---
    
    display_headers = []
    display_rows = []
    
    if not similar_players_df.empty:
        # Pobierz rolę z *wyników*, aby mieć pewność, że jest poprawna
        calculated_role = similar_players_df.iloc[0]['best_role']
        fit_col_name = f'fit_{calculated_role}'
        
        # 1. Zdefiniuj nagłówki
        display_headers = ["Name", "Position", "Best Role", f"Fit ({calculated_role})", "Similarity"]
        
        # 2. Przygotuj wiersze (teraz jako lista SŁOWNIKÓW)
        display_rows = []
        for index, player_data in similar_players_df.iterrows():
            similarity_val = player_data.get('similarity', 0)
            fit_val = player_data.get(fit_col_name, 0)
            
            row = {
                'name': player_data.get('name', ''),
                'position': player_data.get('position', ''),
                'best_role': player_data.get('best_role', ''),
                'fit_score': f"{fit_val:.2f}",
                'similarity': f"{similarity_val:.2f}%",
                # UŻYWAMY NOWEJ FUNKCJI DO PRZYPISANIA KLASY
                'similarity_class': get_similarity_class(similarity_val) 
            }
    ...
    # 3. Wyślij gotowe dane do szablonu
    context = {
        'target_player_name': target_player_name,
        'target_player_role': target_player_role, # Rola gracza-wzoru
...
# ----------------------------------------------------------------------
# NOWA FUNKCJA POMOCNICZA DLA KOLORÓW
# ----------------------------------------------------------------------
def get_similarity_class(similarity_value):
    """Zwraca nazwę klasy CSS na podstawie wartości podobieństwa."""
    if similarity_value >= 95:
        return 'similarity-green'  
    if similarity_value >= 85:
        return 'similarity-yellow' 
