...
ATTRIBUTE_NAME_MAP = {
    ...
    'One on Ones': 'one_on_ones', 'Long Shots': 'long_shots', 'Long Throws': 'long_throws',
    'Off the Ball': 'off_the_ball', 'Work Rate': 'work_rate',
    ...
    'Acceleration': 'acceleration', 'Pace': 'pace', 'Agility': 'agility',
    'Stamina': 'stamina', 'Strength': 'strength', 'Heading': 'heading', 'Marking': 'marking',
    'Anticipation': 'anticipation', 'Tackling': 'tackling', 'Passing': 'passing', 'Technique': 'technique',
    'Corners': 'corners', 'Crossing': 'crossing', 'Dribbling': 'dribbling', 'Finishing': 'finishing', 'Aggression': 'aggression',
...
def fit_player(player_series, role, roles_weight):
    ...
    for attr, weight in key_stats.items():
        col_name = ATTRIBUTE_NAME_MAP.get(attr)
        if col_name is None: 
             col_name = attr.lower().replace(' ', '_')
...
def find_similar(player_name, df, roles_weight):
    ...
    if role_series.empty:
        return pd.DataFrame(), None, f"Nie znaleziono gracza o nazwie '{player_name}' w połączonych plikach."
    
    role = role_series.iloc[0]
    
    if role == "Unknown":
         return pd.DataFrame(), role, f"Nie można porównać gracza '{player_name}', ponieważ ma nieznaną najlepszą rolę."

    df = fit_to_role(role, df, ROLES_WEIGHT)
    ...
    return similar_players, role, None
...
def run_similarity_analysis(request):
    ...
    target_player_role_initial = target_rows[0][CSV_HEADER.index('best_role')]
    ...
    if error:
        ...
        return redirect('main')
    if not base_rows:
        messages.warning(request, f"Plik bazy ({file_base.name}) jest pusty. Nie ma graczy do porównania.")
        base_rows = [] 
    ...
    similar_players_df, calculated_role, error = find_similar(target_player_name, df, ROLES_WEIGHT)
    
    if error:
        messages.error(request, error)
        context = {
            'target_player_name': target_player_name,
            'target_player_role': calculated_role if calculated_role else target_player_role_initial,
            'display_headers': [],
            'display_rows': []
        }
    ...
    target_role_name = calculated_role if calculated_role else target_player_role_initial
    
    if not similar_players_df.empty:
        fit_col_name = f"fit_{target_role_name}" 
        
        display_headers = ["Name", "Position", "Best Role", f"Fit ({target_role_name})", "Similarity"]

        for index, player_data in similar_players_df.iterrows():
            
            similarity_value = player_data.get('similarity', 0)
            sim_class = 'sim-low'
            if similarity_value >= 98:
                sim_class = 'sim-high' 
            elif similarity_value >= 95:
            ...
            row_dict = {
                ...
                'fit_score': f"{player_data.get(fit_col_name, 0):.2f}",
                'similarity': f"{similarity_value:.2f}%",
        ...
        if base_rows:
             messages.warning(request, f"Po usunięciu gracza '{target_player_name}' nie było innych graczy do porównania.")
       
    context = {
        'target_player_name': target_player_name,
        'target_player_role': target_role_name, 
